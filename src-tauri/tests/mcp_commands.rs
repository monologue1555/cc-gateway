...
fn import_default_config_claude_persists_provider() {
    ...
    assert!(
        db_path.exists(),
        "importing default config should persist to cc-gateway.db"
    );
}
...
fn import_mcp_from_claude_creates_config_and_enables_servers() {
    ...
    assert!(
        db_path.exists(),
        "state.save should persist to cc-gateway.db when changes detected"
    );
}
...
fn set_mcp_enabled_for_codex_writes_live_config() {
    ...
    let toml_path = cc_gateway_lib::get_codex_config_path();
    assert!(
        toml_path.exists(),
